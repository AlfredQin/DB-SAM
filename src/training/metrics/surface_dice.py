...
def compute_surface_distances(mask_gt, mask_pred, spacing_mm, verbose=False):
...
class SurfaceDICEScore(Metric):
    def __init__(
        ...
        super().__init__()
        self.add_state("surface_dice_score", default=torch.tensor(0, dtype=torch.float), dist_reduce_fx="sum")
    ...
    def update(self, preds: List[torch.Tensor], target: List[torch.Tensor]) -> None:
        for pred, tgt in zip(preds, target):
            ...
            surface_distance = compute_surface_distances(tgt, pred, spacing_mm=self.spacing_mm)
            surface_dice = compute_surface_dice_at_tolerance(surface_distance, self.tolerance)

            self.surface_dice_score += surface_dice
            self.total += 1

    def compute(self):
        return self.surface_dice_score / self.total
